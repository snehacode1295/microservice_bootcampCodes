...
public class AuthManager {
	...
	UserService userService;
	 ...
	 public Authentication authenticate(UsernamePasswordAuthenticationToken authentication,LoginRequest loginRequest) throws AuthenticationException {
	    String loginID = authentication.getPrincipal() + "";
	    String password = authentication.getCredentials() + "";
	    User user;
		try {
			logger.info("user is going to validate(AuthManager) "+loginID);
			if(userService == null) {
			...
			user = userService.findByLoginID(loginID);
			 ...
			 if(!this.passwordMatch(password, user.getPassword())) {
				 logger.info("Password is wrong for user .."+user.getEmail()+"-- "+user.getLoginID());
				 throw new BadCredentialsException("LoginID or password is wrong.");
			 } 

		        return new UsernamePasswordAuthenticationToken(new UserPrincipal(user.getId(), loginID, password), password);
		} catch (Exception e) {
			logger.info("Error",e);
